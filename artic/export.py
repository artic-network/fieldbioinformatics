...
def run(parser, args):
    ...
    cmd = (
        'fast5_subset -r -i "%s" -l "%s/%s.reads.list" -f "%s_" -n 4000 -s "%s/fast5"'
        % (
    ...
    filtered_summary_file["filename"] = filtered_summary_file.apply(
        lambda row: mapping_file.loc[row["read_id"]]["filename"], axis=1
    )
